...
describe("Test ts-retrofit.", () => {
  ...
  test("Test `@BasePath` decorator.", async () => {
    const userService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(UserService);
  ...
  test("Test `@Headers` decorator.", async () => {
    const authService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(AuthService);
    const auth: Auth = {
      username: "test",
      password: "123456"
    };
  ...
  test("Test `@HeaderMap` decorator.", async () => {
    const postService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(PostService);
  ...
  test("Test `@QueryMap` decorator.", async () => {
    const searchService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(SearchService);
  ...
  test("Test `@FormUrlEncoded` decorator with nested object.", async () => {
    const groupService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(GroupService);
    const group: Group = {
      name: "Video Game",
  ...
  test("Test `@Multipart` decorator.", async () => {
    const fileService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(FileService);
    const bucket = {
      value: "test-bucket"
    };
    const file = {
  ...
  test("Test `@Multipart` decorator 1.", async () => {
    const messagingService = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(MessagingService);
  ...
  test("Test `@Get` decorator and path is absolute URL.", async () => {
    const service = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(UserService);
  ...
  test("Test `@RequestTransformer` decorator.", async () => {
    const service = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(TransformerService);
  ...
  test("Test `@ResponseStatus` decorator.", async () => {
    const service = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(ResponseStatusService);
    expect(service.__meta__.getSomething.responseStatus).toEqual(200);
  });

  test("Test `@Config` decorator.", async () => {
    const service = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(ConfigService);
    const response = await service.getConfig();
    expect(response.config.maxRedirects).toEqual(1);
  });

  test("Test absolute URL.", async () => {
    const service = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(AbsoluteURLService);
    try {
  ...
  test("Test `ignoreBasePath` in HTTP method option.", async () => {
    const service = new ServiceBuilder().setEndpoint(TEST_SERVER_ENDPOINT).build(HttpMethodOptionsService);
